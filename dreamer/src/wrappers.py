...
class RaceCarBaseEnv:
    def __init__(self, track, task, rendering=False):
        ...
        self.id = f"{track}-{uuid.uuid4().hex[:6]}"
...
class RaceCarWrapper:
    ...
    def agent_ids(self):
        # multi-agent ids
        return list(self._env.observation_space.spaces.keys())
    ...
    def observation_space(self):
        ...
        for agent_id, obss in self._env.observation_space.spaces.items():
            ...
            for obs_name, obs_space in obss.spaces.items():
                ...
                agent_space["speed"] = gym.spaces.Box(
    ...
    def action_space(self):
        ...
        for agent_id, act in self._env.action_space.spaces.items():
            flat_action_space[agent_id] = gym.spaces.Box(
                np.append(act["motor"].low, act["steering"].low),
    ...
    def step(self, actions):
        actions = {
            i: {"motor": actions[i][0], "steering": actions[i][1]}
        ...
        for agent_id in self.agent_ids:
            ...
            if "low_res_camera" in obs[agent_id]:
                ...
                obs[agent_id]["image"] = np.zeros((64, 64, 3), dtype=np.uint8)
            obs[agent_id]["is_first"] = False
            obs[agent_id]["is_terminal"] = done[agent_id]
    ...
    def reset(self, **kwargs):
        ...
        for agent_id in self.agent_ids:
            ...
            if "low_res_camera" in obs[agent_id]:
                ...
                obs[agent_id]["image"] = np.zeros((64, 64, 3), dtype=np.uint8)
            obs[agent_id]["is_first"] = True
            obs[agent_id]["is_terminal"] = False
        return obs
...
class ActionRepeat:
    ...
    def step(self, action):
        ...
        while current_step < self._amount and not any(dones.values()):
            ...
            total_rewards = {
                agent_id: total_rewards[agent_id] + rewards[agent_id]
...
class ReduceActionSpace:
    ...
    def _normalize(self, action):
        if isinstance(action, dict):
            ...
            arr = action
        return (arr + 1) / 2 * (self._high - self._low) + self._low

    def step(self, action):
        original = {
            agent_id: self._normalize(action[agent_id])
            for agent_id in self._env.agent_ids
        }
...
class Render:
    ...
    def _reset_videos_dict(self):
        # by default: store birds-eye view on first agent
...
class Collect:
    def __init__(self, env, callbacks=None, precision=32, occupancy_shape=(64, 64, 1)):
        ...
        # in multi-agent: store 1 episode for each agent
        self._episodes = [[] for _ in env.agent_ids]
    ...
    def step(self, action):
        obss, reward, dones, info = self._env.step(action)
        obss = {
            agent_id: {k: self._convert(v) for k, v in obs.items()}
        ...
        for i, agent_id in enumerate(obss.keys()):
            ...
            transition[agent_id]["discount"] = info.get(
                "discount", np.array(1 - float(dones[agent_id]))
            )
            transition[agent_id]["progress"] = (
                info[agent_id]["lap"] + info[agent_id]["progress"] - 1
            )  # first lap is 1
            transition[agent_id]["time"] = info[agent_id]["time"]
            self._episodes[i].append(transition[agent_id])
        if any(dones.values()):
            episodes = [
                {k: [t[k] for t in episode] for k in episode[0]}
                for episode in self._episodes
            ]
            episodes = [
                {k: self._convert(v) for k, v in episode.items()}
    ...
    def reset(self):
        ...
        for i, agent_id in enumerate(obs.keys()):
            transition[agent_id]["action"] = np.zeros(
                self._env.action_space[agent_id].shape
            )
...
class OccupancyMapObs:
    def __init__(self, env, neigh_size=100):
        ...
        # half-size (in pixel) of the observable sub-map centered on the agent
    ...
    def observation_space(self):
        ...
        spaces["lidar_occupancy"] = gym.spaces.Box(
            -np.inf, np.inf, shape=self._occupancy_map_size, dtype=np.uint8
        )
        return gym.spaces.Dict(spaces)
    ...
    def step(self, action):
        ...
        for agent_id in self._env.agent_ids:
            ...
            track_map = self._occupancy_map._map[
                pr - (self._neigh_size + 10) : pr + (self._neigh_size + 10),
                pc - (self._neigh_size + 10) : pc + (self._neigh_size + 10),
            ].copy()
            track_map = track_map.astype(np.uint8)
            track_map = ndimage.rotate(
                track_map, np.rad2deg(2 * np.pi - pose[-1])
            )  # rotate with car orientation
            cr, cc = track_map.shape[0] // 2, track_map.shape[1] // 2
            cropped = track_map[
                cr - self._neigh_size : cr + self._neigh_size,
                cc - self._neigh_size : cc + self._neigh_size,
            ]
            cropped = np.array(
                Image.fromarray(cropped).resize(size=self._map_size[:2])
            )  # resize as 2d image
...
class SingleAgentFlattenWrapper:
    def __init__(self, env, agent_id="A"):
        self._env = env
        self._id = agent_id
    ...
    @property
    def observation_space(self):
        return self._env.observation_space.spaces[self._id]

    @property
    def action_space(self):
        return self._env.action_space.spaces[self._id]

    def step(self, action):
        # Wrap single-agent action in dict
        action = {self._id: action}
        obss, rewards, dones, infos = self._env.step(action)
        return (
            obss[self._id],
            rewards[self._id],
            dones[self._id],
    ...
    def reset(self, **kwargs):
        obss = self._env.reset(**kwargs)
        return obss[self._id]
