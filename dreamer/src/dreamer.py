...
def make_env(config, mode, gui=True):
    ...
    elif mode == "eval":  # This corresponds to make_test_env in function 2
...
def main(config):
    ...
    train_envs = [make_env(config, "train", gui=True) for i in range(config.envs)]
    eval_envs = [make_env(config, "eval", gui=True) for i in range(config.envs)]
    if config.parallel:
